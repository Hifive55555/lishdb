...
impl StorageManager {
    ...
    /// 从指定表的指定列的指定行读取值
    pub fn get_column_value(&self, table_name: &str, column_name: &str, row_index: u64) -> Result<Option<SingleValue>> {
        debug!("从表 {} 的列 {} 的第 {} 行读取值", table_name, column_name, row_index);
        
        let table_path = self.config.data_dir.join(table_name);
        
        // 检查表目录是否存在
        if !table_path.exists() {
            debug!("表目录不存在: {:?}", table_path);
            return Ok(None);
        }
        ...
        // 读取指定列的指定行
        let result = table_file.read_column_value(column_name, row_index)?;
    ...
    pub fn insert_data(&self, table_meta: &TableMeta, column_names: &[String], values: &[Vec<String>]) -> Result<()> {
        debug!("向表 {} 插入 {} 行数据", table_meta.name, values.len());
...
/// 根据数据类型编码值
fn encode_value(value: &str, data_type: &DataType) -> Vec<u8> {
    // 简化实现：直接将字符串转换为字节
    // 实际项目中应该根据具体的数据类型进行正确的编码
    match data_type {
        DataType::Integer => {
            if let Ok(int_val) = value.parse::<i32>() {
                return int_val.to_le_bytes().to_vec();
            }
        },
        DataType::Float => {
            if let Ok(float_val) = value.parse::<f64>() {
                return float_val.to_le_bytes().to_vec();
            }
        },
        DataType::Boolean => {
            let bool_val = value.to_lowercase() == "true" || value == "1";
            return vec![bool_val as u8];
        },
...
impl TableFile {
    ...
    pub fn insert_data(&mut self, table_meta: &TableMeta, column_names: &[String], values: &[Vec<String>]) -> Result<()> {
        debug!("开始插入数据，共 {} 行，每行 {} 列", values.len(), column_names.len());
        
        // 对每一行数据
        for (row_idx, row_values) in values.iter().enumerate() {
            debug!("处理第 {} 行数据", row_idx);
            
            // 对每一列数据
            for (col_idx, col_name) in column_names.iter().enumerate() {
                debug!("处理第 {} 行的列: {}", row_idx, col_name);
                ...
                let column_meta = table_meta.columns.iter()
                    ...
                    .ok_or_else(|| {
                        debug!("未找到列: {}", col_name);
                        crate::error::TableError::ColumnNotFound(col_name.clone())
                    })?;
                ...
                debug!("编码值: {} (类型: {:?})", value, column_meta.data_type);
                let encoded_value = encode_value(value, &column_meta.data_type);
                
                // 写入值的长度作为前缀
                let len_bytes = encoded_value.len() as u32;
                debug!("写入长度前缀: {}", len_bytes);
                file.write_all(&len_bytes.to_le_bytes())?;
                
                // 写入实际值
                debug!("写入实际值，长度: {}", encoded_value.len());
                file.write_all(&encoded_value)?;
    ...
    fn get_or_open_column_file(&mut self, column_name: &str) -> Result<&mut File> {
        debug!("获取或打开列文件: {}", column_name);
        
        // 如果文件已经打开，直接返回
        if self.col_files.contains_key(column_name) {
            debug!("列文件 {} 已打开，直接返回", column_name);
        ...
        // 检查文件是否存在
        let file_exists = column_file_path.exists();
        debug!("文件存在: {}", file_exists);
    ...
    /// 从指定列中读取一行数据
    pub fn read_column_value(&mut self, column_name: &str, row_index: u64) -> Result<Option<SingleValue>> {
        debug!("读取列 {} 的第 {} 行数据", column_name, row_index);
        
        // 确保列文件已打开
        let file = self.get_or_open_column_file(column_name)?;
        ...
        match file.read_exact(&mut len_bytes) {
            Err(e) if e.kind() == std::io::ErrorKind::UnexpectedEof => {
                // 文件过早结束，目标行不存在
                debug!("读取目标行 {} 的长度时文件过早结束", row_index);
                return Ok(None);
            },
            Err(e) => {
                debug!("读取目标行 {} 的长度失败: {:?}", row_index, e);
                return Err(e.into());
            },
            Ok(_) => {}
        }
        ...
        match file.read_exact(&mut buffer) {
            Err(e) if e.kind() == std::io::ErrorKind::UnexpectedEof => {
                // 文件过早结束，值不完整
                debug!("读取目标行 {} 的内容时文件过早结束", row_index);
                return Ok(None);
            },
            Err(e) => {
                debug!("读取目标行 {} 的内容失败: {:?}", row_index, e);
                return Err(e.into());
            },
            Ok(_) => {}
        }
        
        debug!("成功读取目标行 {} 的值", row_index);
        Ok(Some(SingleValue(buffer)))
    }
